...
uint16_t currentRxBuffLen = 0;
uint16_t currentTxBuffLen = 0;
...
void Eth_init_phy()
{
    ...
    Eth_LED_LINKSET(1); /* turn off link led. */
    Eth_LED_DATASET(1); /* turn off data led. */
...
void init_lwip()
{
    ...
    IP4_ADDR(&gw, IP_ADDRESS[0], IP_ADDRESS[1], IP_ADDRESS[2], 1);
...
void Eth_net_led_tmr(void)
{
    ...
    if (net_data_led) {
        ...
        if (net_data_led_require != 0) {
            net_data_led = 1;
            Eth_LED_DATASET(0); /* turn on data led. */
