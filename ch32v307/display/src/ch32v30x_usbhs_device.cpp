...
volatile uint8_t RxBULK_flag = 0;
volatile uint16_t RxBULK_len = 0;
...
void USBHS_Device_Init(FunctionalState sta) {
    if (sta) {
        ...
        USBHSD->CONTROL =
            USBHS_UC_DMA_EN | USBHS_UC_INT_BUSY | USBHS_UC_SPEED_HIGH;
    ...
    if ((endp >= DEF_UEP1) && (endp <= DEF_UEP15)) {
        ...
        if (endp_en & USBHSD_UEP_TX_EN(endp)) {
            if ((USBHS_Endp_Busy[endp] & DEF_UEP_BUSY) == 0x00) {
                ...
                USBHSD_UEP_TXCTRL(endp) =
                    (USBHSD_UEP_TXCTRL(endp) &= ~USBHS_UEP_T_RES_MASK) |
                    USBHS_UEP_T_RES_ACK;
                /* Set end-point busy */
    ...
    if (intflag & USBHS_UIF_TRANSFER) {
        ...
        case USBHS_UIS_TOKEN_IN:
            ...
            case USBHS_UIS_TOKEN_IN | DEF_UEP0:
                ...
                if ((USBHS_SetupReqType & USB_REQ_TYP_MASK) !=
                ...
                if (intst & USBHS_UIS_TOG_OK) {
                    ...
                    if ((USBHS_SetupReqType & USB_REQ_TYP_MASK) !=
                        ...
                        if (USBHS_SetupReqCode == CDC_SET_LINE_CODING) {
                            ...
                            USBHSD->UEP0_RX_CTRL =
                                (USBHSD->UEP0_RX_CTRL & USBHS_UEP_R_TOG_MASK) |
                                USBHS_UEP_R_RES_ACK;
                        }
                    } else {
        ...
        if ((USBHS_SetupReqType & USB_REQ_TYP_MASK) != USB_REQ_TYP_STANDARD) {
                ...
                case HID_GET_REPORT: /* 0x01: GET_REPORT */
                    if (USBHS_SetupReqIndex == 0x00) {
                        ...
                        len = (USBHS_SetupReqLen >= DEF_USBD_UEP0_SIZE)
                                  ? DEF_USBD_UEP0_SIZE
                                  : USBHS_SetupReqLen;
                        memcpy(USBHS_EP0_Buf,
                               &HID_Report_Buffer[Hid_Report_Ptr], len);
                        Hid_Report_Ptr += len;
                    } else {
                        errflag = 0xFF;
                    }
            ...
            len = (USBHS_SetupReqLen >= DEF_USBD_UEP0_SIZE) ? DEF_USBD_UEP0_SIZE
                                                            : USBHS_SetupReqLen;
            memcpy(USBHS_EP0_Buf, pUSBHS_Descr, len);
            pUSBHS_Descr += len;
        } else {
            ...
            case USB_GET_DESCRIPTOR:
                ...
                case USB_DESCR_TYP_CONFIG:
                    ...
                    if ((USBHSD->SPEED_TYPE & USBHS_SPEED_TYPE_MASK) ==
                        USBHS_SPEED_HIGH) {
                        /* High speed mode */
            ...
            case USB_CLEAR_FEATURE:
                if ((USBHS_SetupReqType & USB_REQ_RECIP_MASK) ==
                ...
                } else if ((USBHS_SetupReqType & USB_REQ_RECIP_MASK) ==
                           USB_REQ_RECIP_ENDP) {
                    /* Set End-point Feature */
                    if ((uint8_t)(USBHS_SetupReqValue & 0xFF) ==
            ...
            case USB_GET_STATUS:
                ...
                if ((USBHS_SetupReqType & USB_REQ_RECIP_MASK) ==
                    ...
                    case (DEF_UEP2 | DEF_UEP_OUT):
                        if (((USBHSD->UEP2_RX_CTRL) & USBHS_UEP_R_RES_MASK) ==
                            USBHS_UEP_R_RES_STALL) {
                            USBHS_EP0_Buf[0] = 0x01;
                        }
                        break;
                    ...
                    case (DEF_UEP3 | DEF_UEP_IN):
                        if (((USBHSD->UEP3_TX_CTRL) & USBHS_UEP_T_RES_MASK) ==
                            USBHS_UEP_T_RES_STALL) {
                            USBHS_EP0_Buf[0] = 0x01;
                        }
                        break;
                    ...
                    case (DEF_UEP4 | DEF_UEP_OUT):
                        if (((USBHSD->UEP4_RX_CTRL) & USBHS_UEP_R_RES_MASK) ==
                            USBHS_UEP_R_RES_STALL) {
                            USBHS_EP0_Buf[0] = 0x01;
                        }
                        break;
        ...
        if (USBHSD->MIS_ST & USBHS_UMS_SUSPEND) {
            ...
            if (USBHS_DevSleepStatus == 0x03) { /* Handling usb sleep here */
            }
        } else {
            USBHS_DevSleepStatus &= ~0x02;
        }
