...
/* Serial port 2 transmit data buffer */
__attribute__((aligned(4))) uint8_t Tx_Buf[DEF_UARTx_TX_BUF_LEN];
/* Serial port 2 receive data buffer */
__attribute__((aligned(4))) uint8_t Rx_Buf[DEF_UARTx_RX_BUF_LEN];

volatile uint8_t ReceivedLenEP0 = 0;
volatile uint8_t ReceivedLenEP2 = 0;
/* Serial port 1 receive dma current counter */
volatile uint32_t UARTx_Rx_DMACurCount;
...
void USBFS_Device_Endp_Init(void) {
    ...
    for (i = 0; i < DEF_UEP_NUM; i++) { USBFS_Endp_Busy[i] = 0; }
}
...
void USBFS_Device_Init(FunctionalState sta) {
    if (sta) {
        ...
        USBOTG_FS->INT_EN =
            USBFS_UIE_SUSPEND | USBFS_UIE_BUS_RST | USBFS_UIE_TRANSFER;
        USBOTG_FS->BASE_CTRL =
    ...
    if ((endp >= DEF_UEP1) && (endp <= DEF_UEP7)) {
        if (USBFS_Endp_Busy[endp] == 0) {
            ...
            if (endp_mode & USBFSD_UEP_TX_EN) {
                ...
                /* response ack */
                USBFSD_UEP_CTRL(endp) =
                    (USBFSD_UEP_CTRL(endp) & ~USBFS_UEP_T_RES_MASK) |
                    USBFS_UEP_T_RES_ACK;
                /* Set end-point busy */
    ...
    if (intflag & USBFS_UIF_TRANSFER) {
            ...
            case USBFS_UIS_TOKEN_IN | DEF_UEP0:
                ...
                if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) !=
                    ...
                    case USB_SET_ADDRESS:
                        USBOTG_FS->DEV_ADDR =
                            (USBOTG_FS->DEV_ADDR & USBFS_UDA_GP_BIT) |
                            USBFS_DevAddr;
                        break;
            ...
            case USBFS_UIS_TOKEN_OUT | DEF_UEP0:
                ...
                if (intst & USBFS_UIS_TOG_OK) {
                    if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) !=
                        ...
                        /* Add your code here */
                        ReceivedLenEP0 = USBOTG_FS->RX_LEN;
                        // memcpy(Rx_Buf, USBFS_EP2_Buf, len);
                    }
                    if (USBFS_SetupReqLen == 0) {
                        USBOTG_FS->UEP0_TX_LEN = 0;
                        USBOTG_FS->UEP0_TX_CTRL =
                            USBFS_UEP_T_TOG | USBFS_UEP_T_RES_ACK;
                    }
                }
                break;

            /* end-point 2 data out interrupt (receive data)*/
            case USBFS_UIS_TOKEN_OUT | DEF_UEP2:
                USBOTG_FS->UEP2_RX_CTRL ^= USBFS_UEP_R_TOG;
                {
                    ReceivedLenEP2 = USBOTG_FS->RX_LEN;
                    // TODO: parse in data
        ...
        case USBFS_UIS_TOKEN_SETUP:
            ...
            if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) !=
                ...
                case USB_CLEAR_FEATURE:
                    if ((USBFS_SetupReqType & USB_REQ_RECIP_MASK) ==
                        ...
                        if ((uint8_t)(USBFS_SetupReqValue & 0xFF) ==
                            ...
                            /* clear usb sleep status, device not prepare to
                             * sleep */
                ...
                /* This request allows the host to select another setting for
            ...
            /* errflag = 0xFF means a request not support or some errors
             * occurred, else correct */
            if (errflag == 0xff) {
        ...
        if (USBOTG_FS->MIS_ST & USBFS_UMS_SUSPEND) {
            ...
            if (USBFS_DevSleepStatus == 0x03) { /* Handling usb sleep here */
            }
        } else {
            USBFS_DevSleepStatus &= ~0x02;
        }
