...
#define USBFSD_UEP_CTRL(n)                                                     \
    (*((volatile uint8_t*)(USBFSD_UEP_CTL_BASE + n * 0x04)))
#define USBFSD_UEP_DMA(n)                                                      \
    (*((volatile uint32_t*)(USBFSD_UEP_DMA_BASE + n * 0x04)))
#define USBFSD_UEP_BUF(n)                                                      \
    ((uint8_t*)(*((volatile uint32_t*)(USBFSD_UEP_DMA_BASE + n * 0x04))) +     \
     0x20000000)
#define USBFSD_UEP_TLEN(n)                                                     \
    (*((volatile uint16_t*)(USBFSD_UEP_LEN_BASE + n * 0x04)))

/* Setup Request Packets */
#define pUSBFS_SetupReqPak ((PUSB_SETUP_REQ)USBFS_EP0_Buf)

#define DEF_UARTx_RX_BUF_LEN (1 * 64) /* Serial x receive buffer size */
#define DEF_UARTx_TX_BUF_LEN (1 * 64) /* Serial x transmit buffer size */
/*******************************************************************************/
/* Variable Definition */
/* Global */
...
uint8_t Tx_Buf[DEF_UARTx_TX_BUF_LEN]; /* Serial x transmit buffer */
extern __attribute__((aligned(4)))
uint8_t Rx_Buf[DEF_UARTx_RX_BUF_LEN]; /* Serial x transmit buffer */
