...
void Sd::SDIO_Clock_Set(u8 clkdiv) {
...
SD_Error Sd::SD_PowerOFF(void) {
    SDIO_SetPowerState(SDIO_PowerState_OFF);
    return SD_OK;
}

SD_Error Sd::SD_InitializeCards(void) {
    SD_Error errorstatus = SD_OK;
    u16 rca = 0x01;
...
SD_Error Sd::SD_GetCardInfo(SD_CardInfo* cardinfo) {
    SD_Error errorstatus = SD_OK;
    u8 tmp = 0;
        ...
        (SDIO_MULTIMEDIA_CARD == CardType)) //标准1.1/2.0卡/MMC卡
    {
        ...
        cardinfo->CardCapacity =
            (long long)(cardinfo->SD_csd.DeviceSize + 1) * 512 * 1024;
...
SD_Error Sd::SD_EnableWideBusOperation(u32 wmode) {
    SD_Error errorstatus = SD_OK;
    if (SDIO_MULTIMEDIA_CARD == CardType) {
...
SD_Error Sd::SD_SetDeviceMode(u32 Mode) {
    SD_Error errorstatus = SD_OK;
...
SD_Error Sd::SD_SelectDeselect(u32 addr) {
...
SD_Error Sd::SD_ReadBlock(u8* buf, long long addr, u16 blksize) {
    SD_Error errorstatus = SD_OK;
    u8 power;
    ...
    if (DeviceMode == SD_POLLING_MODE) {
        ...
        SDIO_DMACmd(ENABLE);
        while (((DMA2->INTFR & 0X2000) == RESET) && (TransferEnd == 0) &&
               (TransferError == SD_OK) && timeout) {
            timeout--;
        }
        if (timeout == 0) { return SD_DATA_TIMEOUT; }
...
SD_Error Sd::SD_ReadMultiBlocks(u8* buf, long long addr, u16 blksize,
                                u32 nblks) {
    SD_Error errorstatus = SD_OK;
    u8 power;
    u32 count = 0;
...
SD_Error Sd::SD_WriteBlock(u8* buf, long long addr, u16 blksize) {
    SD_Error errorstatus = SD_OK;
...
SD_Error Sd::SD_WriteMultiBlocks(u8* buf, long long addr, u16 blksize,
                                 u32 nblks) {
    SD_Error errorstatus = SD_OK;
    u8 power = 0, cardstate = 0;
    u32 timeout = 0, bytestransferred = 0;
...
SD_Error Sd::CmdResp7Error(void) {
    SD_Error errorstatus = SD_OK;
    u32 status;
    u32 timeout = SDIO_CMD0TIMEOUT;
    while (timeout--) {
...
SD_Error Sd::CmdResp1Error(u8 cmd) {
    u32 status;
    while (1) {
        status = SDIO->STA;
        if (status & ((1 << 0) | (1 << 2) | (1 << 6))) { break; }
...
SD_Error Sd::CmdResp3Error(void) {
    u32 status;
    while (1) {
        status = SDIO->STA;
        if (status & ((1 << 0) | (1 << 2) | (1 << 6))) { break; }
...
SD_Error Sd::CmdResp2Error(void) {
    SD_Error errorstatus = SD_OK;
    u32 status;
    u32 timeout = SDIO_CMD0TIMEOUT;
    while (timeout--) {
...
SD_Error Sd::CmdResp6Error(u8 cmd, u16* prca) {
    SD_Error errorstatus = SD_OK;
    u32 status;
    u32 rspr1;
    while (1) {
    ...
    rspr1 = SDIO->RESP1;
    if (SD_ALLZERO == (rspr1 & (SD_R6_GENERAL_UNKNOWN_ERROR |
...
SD_Error Sd::SDEnWideBus(u8 enx) {
    SD_Error errorstatus = SD_OK;
    u32 scr[2] = {0, 0};
    u8 arg = 0X00;
    if (enx) {
...
SD_Error Sd::IsCardProgramming(u8* pstatus) {
...
SD_Error Sd::SD_SendStatus(uint32_t* pcardstatus) {
    SD_Error errorstatus = SD_OK;
...
SDCardState Sd::SD_GetState(void) {
    u32 resp1 = 0;
...
u8 Sd::convert_from_bytes_to_power_of_two(u16 NumberOfBytes) {
    u8 count = 0;
...
void Sd::SD_DMA_Config(u32* mbuf, u32 bufsize, u32 DMA_DIR) {
    ...
    DMA_InitStructure.DMA_PeripheralBaseAddr =
        reinterpret_cast<uint32_t>(&SDIO->FIFO);
...
u8 Sd::SD_ReadDisk(u8* buf, u32 sector, u8 cnt) {
    u8 sta = SD_OK;
...
u8 Sd::SD_WriteDisk(u8* buf, u32 sector, u8 cnt) {
    u8 sta = SD_OK;
    u8 n;
    long long lsector = sector;
    lsector <<= 9;
