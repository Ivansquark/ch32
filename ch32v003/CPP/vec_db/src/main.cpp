...
int main(void) {
    ...
    std::vector<Map>* db =
        new std::vector<Map>{{"RULON", "OBOEV"}, {"KAMAZ", "POMOEV"}};
    ...
    while (1) {
        if (Uart::IsReceived) {
            ...
            if (parse == "REB") {
                ...
                printStr(str);
                int count = 0;
                for (std::vector<Map>::iterator it = db->begin();
                     it != db->end(); ++it, count++) {
                    if ((*it).first == str) {
                        printStr((*it).first);
                        printAddr("it = 0x", (uint32_t)&it);
                        db->erase(it); // invalidate iterator => break;
                        break;
                    }
                }
            } else if (parse == "INT") {
