...
char Uart::data[32] = {0};
char Uart::rxdata[32] = {0};
uint8_t Uart::counter = 0;
bool Uart::IsReceived = false;

Uart::Uart() { init(); }

void Uart::sendStr(char* arr, int len) {
    for (int i = 0; i < len; i++) {
        sendbyte(arr[i]);
    }
}
...
void USART1_IRQHandler(void) {
    if (USART1->STATR & USART_STATR_RXNE) {
        ...
        char c = USART1->DATAR;
        Uart::data[Uart::counter++] = c;
        Uart::sendStr(&c, 1);
        if(Uart::data[Uart::counter - 1] == '\n') {
            //parse
            memcpy(Uart::data, Uart::rxdata, Uart::counter);
            Uart::counter = 0;
